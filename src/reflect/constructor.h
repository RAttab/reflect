...
{
    type->add(type->id().c_str(), [] { return T(); });
}
...
{
    type->add(type->id().c_str(), [] (const T& other) { return T(other); });
}
...
{
    type->add(type->id().c_str(), [] (T&& other) { return T(std::move(other)); });
}
...
{
    type->add(type->id().c_str(), [] (Args... args) {
